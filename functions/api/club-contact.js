// Dedicated club LP endpoint. Generated from contact.js with notification labels only changed.
const JSON_HEADERS = {
...
export async function onRequestPost({ request, env }) {
  ...
  const subject = `【部活動LP】${selectedRequests}／${organization}`.slice(0, 180);
  const safeMessage = escapeHtml(message).replace(/\n/g, '<br>');
  const safeOrganization = escapeHtml(organization);
  const safeName = escapeHtml(name);

  const adminHtml = `
    <h2>部活動LPからのお問い合わせ</h2>
